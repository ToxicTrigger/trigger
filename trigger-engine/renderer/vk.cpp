...
static void FramePresent(ImGui_ImplVulkanH_Window* wd, VkQueue present, vk* vk)
{
	VkSemaphore render_complete_semaphore = wd->FrameSemaphores[wd->SemaphoreIndex].RenderCompleteSemaphore;
	...
	VkResult err = vkQueuePresentKHR(present, &info);
...
void vk::resize()
{
	...
	g_MainWindowData.ImageCount = (uint32_t)swapChainImages.size() - 1;
	{
		...
		g_MainWindowData.RenderPass = renderPass;
		g_MainWindowData.FrameSemaphores = new ImGui_ImplVulkanH_FrameSemaphores[this->imageAvailableSemaphores.size()];
		for (int i = 0; i < imageAvailableSemaphores.size(); ++i)
		{
			g_MainWindowData.FrameSemaphores[i].ImageAcquiredSemaphore = this->imageAvailableSemaphores[i];
			g_MainWindowData.FrameSemaphores[i].RenderCompleteSemaphore = this->renderFinishedSemaphores[i];
		}
	}
	g_Queue = graphicsQueue;
...
void trigger::rend::vk::createLogicalDevice()
{
	...
	//vkGetDeviceQueue(device, indices.)
}
...
void trigger::rend::vk::createDescriptorSetLayout()
{
...
void trigger::rend::vk::createUniformBuffers()
{
...
void trigger::rend::vk::createDescriptorSets()
{
...
void trigger::rend::vk::createTextureImageView()
{
	textureImageView = createImageView(textureImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
}

VkImageView trigger::rend::vk::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels)
{
...
void trigger::rend::vk::createTextureSampler()
{
...
void trigger::rend::vk::createDepthResources()
{
...
VkFormat trigger::rend::vk::findDepthFormat()
{
...
#include "../lib/vk/include/stb/stb_image.h"
void trigger::rend::vk::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
{
...
bool trigger::rend::vk::hasStencilComponent(VkFormat format)
{
	return format == VK_FORMAT_D32_SFLOAT_S8_UINT || format == VK_FORMAT_D24_UNORM_S8_UINT;
}

void trigger::rend::vk::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
{
...
void trigger::rend::vk::createColorResources()
{
...
VkSampleCountFlagBits trigger::rend::vk::getMaxUsableSampleCount()
{
...
VkCommandBuffer trigger::rend::vk::beginSingleTimeCommands()
{
	VkCommandBufferAllocateInfo allocInfo = {};
	allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
	allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
...
void trigger::rend::vk::endSingleTimeCommands(VkCommandBuffer commandBuffer)
{
...
void trigger::rend::vk::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels)
{
	VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void trigger::rend::vk::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
{
	VkCommandBuffer commandBuffer = beginSingleTimeCommands();
